...
- Every claim declares one of two verification levels, and the same two are open to every vendor. **CI-verified** claims ship a snippet this registry executes on every push ([`test_repros.py`](test_repros.py) runs the exact snippet off each page and asserts the block fires *and* that the tool body never ran); only these count toward the coverage totals. **Vendor-attested** claims are verified by the vendor against its own component, render labelled as such, and do not count. `validate()` applies one rule to the maintainer and to everyone else. A claim that stops passing is **withdrawn, not reworded** ([`GOVERNANCE.md`](GOVERNANCE.md)).
