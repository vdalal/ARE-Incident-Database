...
Each incident is a registry fact: what happened, its OWASP ASI category, and the control architecture it requires (its coverage class). Whether a specific product stops it is a vendor claim, shown in the last column and detailed, attributed, on each entry's page.
